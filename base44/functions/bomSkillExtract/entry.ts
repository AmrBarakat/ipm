...
Deno.serve(async (req) => {
  try {
    ...
    for (const row of dataRows) {
      ...
      const marginPct = toNumber(get(row, 'margin', fieldMap));
      let unitCost = toNumber(get(row, 'planned_cost_unit', fieldMap));
      ...
      if (unitSell == null && unitCost != null) {
        ...
        if (!fieldMap.unit_sell) warnings.push(`"${description}": sell price derived from cost × ${factor.toFixed(2)} (not in file)`);
      }
      ...
      const item = { description, partNo, supplier, qty, unit: 'pcs', marginPct, unitCost, totalCost, unitSell, totalSell, category };
    ...
    for (const group of groups) {
      if (group.isPanel && group.items.length > 0) {
        ...
        const totalCost = group.items.reduce((s, i) => s + (i.unitCost != null ? i.unitCost * i.qty : 0), 0);
