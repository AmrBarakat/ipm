...
/**
 * extractDocumentData — generic document extractor for INVOICES, CONTRACTS, and
 * other non-PO/DN documents. Purchase orders and delivery notes are handled by
 * the dedicated extractPODN pipeline; this function no longer extracts them.
 */
Deno.serve(async (req) => {
  try {
    ...
    const result = await base44.asServiceRole.integrations.Core.InvokeLLM({
      ...
      prompt: `You are an expert at parsing industrial project documents (invoices, contracts, and general project documents).
...
- document_type: detected type (invoice, contract, other)
