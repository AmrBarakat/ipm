    ...
    <div>
      ...
      {showForm && (
        <form onSubmit={upload} className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-4 grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <input value={form.reference_number} onChange={e => setForm(f => ({ ...f, reference_number: e.target.value }))}
            placeholder="Reference number" className={inp} />
          ...
          <input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
            placeholder="Description" className={inp + ' md:col-span-2'} />
      ...
      {docs.length === 0 ? (
        ...
        <div className="space-y-2">
          {docs.map(doc => (
            <div key={doc.id} className="bg-white rounded-lg shadow-sm px-4 py-3 flex flex-wrap items-center gap-4">
