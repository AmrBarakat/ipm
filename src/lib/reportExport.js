...
export function drawRow(doc, x, topY, widths, columns, row, size, totalW, sectionTitle) {
  ...
  const rowStyle = row._bold ? 'bold' : 'normal';
  const indent = row._indent || 0;
  let cx = x;
  columns.forEach((c, i) => {
    const cw = widths[i];
    const inner = cw - 2 * PAD_X - (i === 0 ? indent : 0);
    const lines = wrap(doc, row[c.key], inner, size, rowStyle);
    const align = c.align === 'right' ? 'right' : 'left';
    const cellColor = c.cellColor ? c.cellColor(row) : null;
    setColor(doc, cellColor || C.ink, 'text');
    const cellX = cx + PAD_X + (i === 0 ? indent : 0);
    drawLines(doc, lines, cellX, topY + PAD_Y, size, align, cx + cw - PAD_X);
    cx += cw;
  });
}
...
export function sectionTitle(doc, text, x, y, rightX) {
  ...
  if (rightX) {
    ...
    setColor(doc, C.hairline, 'draw');
    doc.setLineWidth(0.1);
    doc.line(x + 5 + tw + 2, y - 2, rightX, y - 2);
  }
}
...
export function exportSectionsPDF(filename, reportTitle, sections, opts = {}) {
  ...
  const genLine = opts.generatedBy
    ? `Generated by ${opts.generatedBy} · ${new Date().toLocaleDateString('en-GB')}`
  ...
  // Project code watermark (large ghosted text on the right)
  if (opts.projectCode) {
    doc.setFont(FONT, 'bold');
    doc.setFontSize(36);
    setColor(doc, [31, 41, 55], 'text'); // slate-800 over dark band
    doc.text(String(opts.projectCode), W - margin, bandHeight / 2 + 6, { align: 'right' });
  }
  ...
  function renderTable(section) {
    ...
    section.rows.forEach((row, i) => {
      ...
      // Row-level fill (_fill takes priority) + zebra + hairline before text.
      const rowFill = row._fill || (i % 2 === 0 ? C.zebra : null);
      if (rowFill) { setColor(doc, rowFill, 'fill'); doc.rect(margin, rowTop, colW, rh, 'F'); }
      setColor(doc, C.hairline, 'draw'); doc.setLineWidth(0.1);
      doc.line(margin, rowTop + rh, margin + colW, rowTop + rh);
  ...
  function renderKpis(s) {
    if (!s.cards || !s.cards.length) { y += 3; return; }
    const cardH = drawKpiCards(doc, margin, y, colW, s.cards, { maxH: maxY - y });
    y += cardH + 5;
  }
  ...
  function renderChart(s) {
    ...
    switch (s.chart) {
      case 'donut': {
        const r = opts.r || 16;
        chartH = drawDonut(doc, margin + r + 5, y + r, r, data.pct ?? 0, { ...opts, maxH: maxY - y });
        break;
      }
      case 'hbars':
        chartH = drawHBars(doc, margin, y, colW, data.rows || [], { ...opts, maxH: maxY - y });
        break;
      case 'stacked':
        chartH = drawStackedBar(doc, margin, y, colW, opts.h || 6, data.segments || [], { ...opts, maxH: maxY - y });
        break;
      case 'line':
        chartH = drawLineChart(doc, margin, y, colW, opts.h || 60, data.series || [], { ...opts, maxH: maxY - y });
        break;
      case 'gauge':
        chartH = drawGauge(doc, margin, y, colW, data.value ?? 0, data.thresholds || { green: 60, amber: 80 }, { ...opts, maxH: maxY - y });
        break;
      default:
        chartH = 0;
    }
  ...
  function renderCallout(s) {
    const h = drawCallout(doc, margin, y, colW, s.tone || 'good', s.title || '', s.body || '', { maxH: maxY - y });
    y += h + 5;
  }

  sections.forEach((s) => {
    ensureSpace(14);
    sectionTitle(doc, s.title, margin, y, margin + colW);
    y += 8;

    if (s.type === 'kpis') {
      renderKpis(s);
    } else if (s.type === 'chart') {
      // Reserve height; page-break before drawing if it doesn't fit, never split.
      const estimated = s.opts?.estimatedH || s.opts?.h || 50;
      ensureSpace(estimated);
      renderChart(s);
    } else if (s.type === 'callout') {
      renderCallout(s);
    } else if (s.type === 'summary' && s.summary) {
  ...
  // ── Footer (accent rule + Confidential left + page numbers right) ────────────
  ...
  for (let p = 1; p <= pc; p++) {
    ...
    // Hairline beneath accent
    setColor(doc, C.hairline, 'draw');
    doc.setLineWidth(0.1);
    doc.line(margin, pageH - 14.5, W - margin, pageH - 14.5);

    doc.setFontSize(SIZES.footer);
    doc.setFont(FONT, 'normal');
    setColor(doc, C.muted, 'text');
    // Confidential — internal use (left)
    doc.text('Confidential — internal use', margin, pageH - 9);
    // Page numbers (right)
    doc.text(`Page ${p} of ${pc}`, W - margin, pageH - 9, { align: 'right' });
  }
